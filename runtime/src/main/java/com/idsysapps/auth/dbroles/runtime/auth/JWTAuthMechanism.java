...
public class JWTAuthMechanism implements AuthenticationMechanism {
  ...
  /**
   * Extract the Authorization header and validate the bearer token if it exists. If it does, and is
   * validated, this builds the org.jboss.security.SecurityContext authenticated Subject that drives
   * the container APIs as well as the authorization layers.
   *
   * @param exchange - the http request exchange object
   * @param securityContext - the current security context that
   * @return one of AUTHENTICATED, NOT_AUTHENTICATED or NOT_ATTEMPTED depending on the header and
      ...
      for (String current : authHeaders) {
        if (current.toLowerCase(Locale.ENGLISH).startsWith("bearer ")) {
          ...
          try {
            ...
            JWTCredential credential = new JWTCredential(bearerToken);
            if (UndertowLogger.SECURITY_LOGGER.isTraceEnabled()) {
              UndertowLogger.SECURITY_LOGGER.tracef("Bearer token: %s", bearerToken);
            }
            ...
            if (account != null) {
              ...
              securityContext.authenticationComplete(jwtAccount, "MP-JJWT", false);
              /*
               * // Workaround authenticated JsonWebToken not being installed as user principal //
               * https://issues.jboss.org/browse/WFLY-9212 org.jboss.security.SecurityContext jbSC =
               * SecurityContextAssociation.getSecurityContext(); Subject subject =
               * jbSC.getUtil().getSubject(); jbSC.getUtil().createSubjectInfo(jwtPrincipal,
               * bearerToken, subject); RoleGroup roles = extract(subject);
               * jbSC.getUtil().setRoles(roles);
               */
              UndertowLogger.SECURITY_LOGGER.debugf(
                  "Authenticated caller(%s) for path(%s) with roles: %s", credential.getName(),
                  exchange.getRequestPath(), account.getRoles());
   ...
   * @return RoleGroup from "Roles" protected RoleGroup extract(Subject subject) {
   *         Optional<Principal> match = subject.getPrincipals() .stream() .filter(g ->
   *         g.getName().equals(SecurityConstants.ROLES_IDENTIFIER)) .findFirst(); Group rolesGroup
   *         = (Group) match.get(); RoleGroup roles = new SimpleRoleGroup(rolesGroup); return roles;
   *         }
   */
}
