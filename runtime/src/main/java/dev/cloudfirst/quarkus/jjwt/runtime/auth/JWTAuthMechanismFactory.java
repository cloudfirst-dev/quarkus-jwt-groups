...
public class JWTAuthMechanismFactory implements AuthenticationMechanismFactory {
  ...
  /**
   * This builds the JWTAuthMechanism with a JWTAuthContextInfo containing the issuer and signer
   * public key needed to validate the token. This information is currently taken from the query
   * parameters passed in via the web.xml/login-config/auth-method value, or via CDI injection.
   *
   ...
   * @param properties - the query parameters from the web.xml/login-config/auth-method value. We
   *        look for an issuedBy and signerPubKey property to use for token validation.
